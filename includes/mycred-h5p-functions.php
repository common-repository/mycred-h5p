	...
	function mycred_h5p_specific_hook_setting( $data, $obj ){
		...
		foreach ( $data as $hook ) {
			...
			if ( ! empty( $h5p_posts ) ) {
				foreach ( $h5p_posts as $post ) {
					$h5p_options .= '<option value="'.esc_attr($post->id).'" '.selected( $hook['content'], $post->id, false ).' >'.esc_html($post->title).'</option>';
				}
			}
		?>
		<div class="hook-instance">
			<div class="row">
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
					<div class="form-group">
						...
						<input type="text" name="<?php echo esc_attr(mycred_h5p_field_name( $obj->mycred_type, 'creds' )); ?>" value="<?php echo esc_attr( $obj->core->number( $hook['creds'] )); ?>" class="form-control mycred-h5p-creds" />
					</div>
				</div>
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
					<div class="form-group">
						<label><?php esc_html_e( 'Log Template', 'mycred' ); ?></label>
						<input type="text" name="<?php echo esc_attr(mycred_h5p_field_name( $obj->mycred_type, 'log' )); ?>" placeholder="<?php esc_attr_e( 'required', 'mycred' ); ?>" value="<?php echo esc_attr( $hook['log'] ); ?>" class="form-control mycred-h5p-log" />
			...
			<div class="row">
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
					<div class="form-group">
						<label><?php esc_html_e( 'Select Specific Content', 'mycred' ); ?></label>
						<select class="form-control mycred-h5p-content" name="<?php echo esc_attr(mycred_h5p_field_name( $obj->mycred_type, 'content' )); ?>">
							<?php echo wp_kses($h5p_options,$select_dropdown_parm); ?>
						</select>
				...
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
					<div class="form-group">
						<label><?php esc_html_e( 'Required Percentage', 'mycred' ); ?></label>
						<input type="text" name="<?php echo esc_attr(mycred_h5p_field_name( $obj->mycred_type, 'percentage' )); ?>" value="<?php echo esc_attr($obj->core->number( $hook['percentage'] )); ?>" class="form-control mycred-h5p-percentage" />
						<span class="description"><?php esc_html_e( 'Use zero to disable.', 'mycred' ); ?></span>
					</div>
				</div>
			</div>
		...
		if ( class_exists( 'myCRED_Addons_Module' ) ) {
        	...
        	if ( $mycred_modules->is_active( 'badges' ) && $screen->id == MYCRED_BADGE_KEY ) {?>
			    ...
			    <?php
					...
					echo "var mycred_badge_completing_h5p_content_specific = '".esc_html($data)."';";
			    ?>
			    </script>
