	...
	final class myCRED_H5P_Core {
		...
		public function required_plugin_notices() {
			...
			if ( !is_plugin_active('mycred/mycred.php') ) {
				printf( '<div class="notice notice-error"><p><a href="https://wordpress.org/plugins/mycred/">%1$s</a> %2$s</p></div>', esc_html_e( 'myCred', 'mycred_h5p' ), esc_html( $msg ) );
			}
			if ( !is_plugin_active('h5p/h5p.php') ) {
				$h5p_msg = __( 'H5P need to be active and installed to use myCred H5P plugin.', 'mycred_h5p' );
				printf( '<div class="notice notice-error"><p><a href="https://wordpress.org/plugins/h5p/">%1$s</a> %2$s</p></div>', esc_html_e( 'H5P', 'mycred_h5p' ), esc_html( $msg ) );
			}
		}
